...
legend('Block Correlation (Baseline)', 'Deep Learning', 'jakes', 'Location', 'southwest');
% --- TẠO ĐỒ THỊ PHỤ (INSET) ĐỂ SOI RÕ VÙNG SNR CAO ---
axes('Position', [0.6, 0.5, 0.25, 0.25]); % [x, y, độ rộng, độ cao] - tùy chỉnh để không đè lên đường chính
box on;
% Vẽ lại 3 đường với cùng màu và ký hiệu
semilogy(gamma_dB, OP_BC_avg, 'r--o', 'LineWidth', 1.5); hold on;
semilogy(gamma_dB, OP_ML_avg, 'b-s', 'LineWidth', 1.5);
semilogy(gamma_dB, OP_Jakes, 'g-*', 'LineWidth', 1.5);

% Thiết lập giới hạn vùng cần soi (Ví dụ SNR từ 10 đến 15)
xlim([10, 15]); 
ylim([min(OP_ML_avg(gamma_dB>=10))*0.5, max(OP_BC_avg(gamma_dB>=10))*2]); 

grid on;
set(gca, 'FontSize', 8); % Chữ nhỏ cho đồ thị phụ
title('Zoomed-in View', 'FontSize', 9);
fprintf('\nMô phỏng hoàn tất!\n');